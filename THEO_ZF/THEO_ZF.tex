...
\begin{itemize}
	\item \textbf{Zerlegungssatz} sagt, dass die Berechnung von einem Wort $(q,w,Z_{1 \ldots k}) \rightarrow^n_M (q',\epsilon,\epsilon)$ mit $w = u_1 \ldots u_k$, $Z_i \in \Gamma^*$ \textbf{sich in $k$ Teile zerlegen lässt}, indem man bei jeder Zerlegung \textbf{nur ein Teil} von $w$ liest und zum Lesen \textbf{beliebig viele} PUSH- und POP-Operationen macht \textbf{ohne den Kellerteil von der folgenden Zerlegung zu berühren (Erweiterungslemma)}.

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.6\linewidth]{zerlegungssatz}
