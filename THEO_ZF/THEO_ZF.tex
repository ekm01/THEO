...
\subsection{Kellerautomat (PDA)} % (fold)
...
\begin{itemize}
	\item \textbf{Idee:}
		\begin{enumerate}
			\item Sobald $M$ einen Endzustand erreicht, darf er den Keller leeren $\rightarrow$ $M'$ \textbf{geht in den neuen Zustand} $\overline{q}$ und \textbf{leert dort den Keller}
...
\item $\delta'(f, \epsilon, Z) = \delta(f, \epsilon, Z) \ \cup \ \{(\overline{q}, Z)\}$ für $Z \in \Gamma$ $\rightarrow$ \textbf{Alle $\epsilon$-Übergänge von Endzuständen zu $q$s und zu $\overline{q}$}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.8\linewidth]{endtoleerpda.jpg}
