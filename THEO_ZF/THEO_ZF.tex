...
\begin{itemize}
  \item Sei $G$ eine Grammatik: $S \rightarrow \epsilon \ | \ [S] \ | \ SS$ 
\item Um zu zeigen, dass $G$ die Menge aller balancierten Klammerwörter erzeugt, betrachten wir die Grammatik als \textbf{induktive Definition einer Sprache $L_G(S)$}
\item \textbf{Induktive Definition von $L_G(S):$}
	\begin{figure}[h!]
	  \centering
	  \includegraphics[width=0.55\linewidth]{induktivedef2}
	\end{figure}
...
\begin{itemize}
  \item Um zu zeigen, dass für alle $u \in L_G(S)$ eine Eigenschaft $P(u)$ gilt, zeige:
	  \begin{figure}[h!]
	    \centering
	    \includegraphics[width=0.45\linewidth]{induktivedef1}
	    \label{fig:induktivedef1}
	  \end{figure}


\item \textit{\underline{Beispiel:}} Grammatik $S \rightarrow \epsilon \ | \ [S] \ | \ SS$ erzeugt genau die Menge der balancierten Wörter
\end{itemize}
